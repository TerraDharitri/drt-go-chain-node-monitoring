...
Node monitoring tool - a lightweight monitoring tool for drt-go-chain node.
...
For now it has a single client which checks a node (or nodes) temp rating via dharitri api (select node by public key).
